...
/**
 * Cancel Order
 * Updates order status to 'Cancelled' and payment status to 'Refunded' if current status is 'Preparing'
...
try {
    ...
    $checkSql = "SELECT OrderStatus, OrderDate FROM orders 
                 WHERE OrderID = ? AND CustomerName = (SELECT CONCAT(FirstName, ' ', LastName) FROM customer WHERE CustomerID = ?)";
    ...
    // Check if order can be cancelled (must be Preparing)
    // FIXED: Only use OrderStatus
    $currentStatus = $order['OrderStatus'];
    
    if (strtolower($currentStatus) !== 'preparing') {
        $conn->rollback();
        http_response_code(400);
        echo json_encode([
            'success' => false,
            'message' => 'Only preparing orders can be cancelled. Current status: ' . $currentStatus
        ]);
        exit;
    }
    ...
    */
    ...
    // Removed payment update as it's now handled by the order's PaymentStatus
