...
try {
    // Fetch orders for this customer directly using CustomerID
    $sql = "SELECT OrderID, OrderDate, OrderTime, TotalAmount, 
                   COALESCE(OrderStatus, 'Preparing') as OrderStatus, 
                   OrderType, 'N/A' as DeliveryOption, Remarks as DeliveryAddress
            FROM orders 
            WHERE CustomerID = ?
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Fetch items for this order using orderdetails table
        $item_sql = "SELECT p.ProductName, od.Quantity, od.UnitPrice 
